...
module alu_16bit (
    ...
    // Internal Signals
    //--------------------------------------------------------------------------
    reg [16:0] result_full;  // Extended result for carry detection
    ...
    //--------------------------------------------------------------------------
    always @(*) begin
        // Extract result (lower 16 bits)
        result = result_full[15:0];

        // Zero flag: set when result is zero
        zero = (result == 16'b0);

        // Carry flag: set from extended bit (for ADD/SUB)
        carry = result_full[16];
        ...
        case (opcode)
            OP_ADD: overflow = (a[15] == b[15]) && (result[15] != a[15]);
            OP_SUB: overflow = (a[15] != b[15]) && (result[15] != a[15]);
